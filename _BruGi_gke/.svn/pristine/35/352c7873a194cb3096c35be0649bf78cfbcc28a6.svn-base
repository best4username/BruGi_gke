...
#if defined(GKE)
  fp_mpu_temp = mpu.readRealTemperature_gke(); // TODO: move to low priority
  for (uint8_t a = 0;a<3;a++)
    axisRot[a] -= config.gyrOffset[a]  + config.gyrM[a] * (fp_mpu_temp - config.gyrTRef);
#else 
  for (uint8_t a = 0;a<3;a++)
    axisRot[a] -= config.gyrOffset[a];
#endif
...
void readACCs()
{
  ...
  devVal[sensorDef.Acc[ROLL].idx]  = rawVal[0] - config.accOffset[0];
  devVal[sensorDef.Acc[PITCH].idx] = rawVal[1] - config.accOffset[1];
  devVal[sensorDef.Acc[YAW].idx]   = rawVal[2] - config.accOffset[2];
